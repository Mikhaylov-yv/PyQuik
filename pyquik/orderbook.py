...
class OrderBook:
    ...
    def get_orderBook(self):
        ...
        self.getRequest(cmd='getParamEx',data = self.clientCode + '|' + self.securityCode + '|' + 'SEC_PRICE_STEP')
        # Запрос: {"data": "CETS|USD000UTSTOM|SEC_SCALE", "id": 9, "cmd": "getParamEx", "t": 1587660869561}
        self.getRequest(cmd='getParamEx',data = self.clientCode + '|' + self.securityCode + '|' + 'SEC_SCALE')
        # Запрос: {"data":"CETS|USD000UTSTOM|LOTSIZE","id":10,"cmd":"getParamEx","t":1587660869562}
        self.getRequest(cmd='getParamEx', data=self.clientCode + '|' + self.securityCode + '|' + 'LOTSIZE')
        # Запрос: {"data":"CETS|USD000UTSTOM","id":13,"cmd":"Subscribe_Level_II_Quotes","t":1587660869576}
        self.getRequest(cmd='Subscribe_Level_II_Quotes', data=self.classCode + '|' + self.securityCode )
        # Запрос: {"data":"CETS|USD000UTSTOM","id":14,"cmd":"IsSubscribed_Level_II_Quotes","t":1587660869588}
        self.getRequest(cmd='IsSubscribed_Level_II_Quotes', data=self.classCode + '|' + self.securityCode)
        # Запрос: {"data":"CETS|USD000UTSTOM","id":14,"cmd":"IsSubscribed_Level_II_Quotes","t":1587660869588}
        self.getRequest(cmd='IsSubscribed_Level_II_Quotes', data=self.classCode + '|' + self.securityCode)
        print('==========================\n==========================')
        ...
        while (True):
            ...
            self.getRequest(cmd='getParamEx', data=self.classCode + '|' + self.securityCode + '|' + 'LAST')
            ...
            self.getRequest(cmd='getDepoEx', data=self.firmid + '|' + self.clientCode + '|'
                                                  + self.securityCode + '|' + self.tradeAccount + '|2')
            time.sleep(1)
